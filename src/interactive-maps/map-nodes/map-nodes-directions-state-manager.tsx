...
const mapNodeDirectionsReducer = (
  state: MapNodeDirections,
  action: Actions
) => {
  const { type } = action
  switch (type) {
...
const mapNodesReducer = (state: State, action: Actions) => {
  ...
  switch (type) {
    ...
    case 'DELETE_DIRECTION': {
      ...
      if (mapNode) {
        const updatedMapNodeDirections = mapNodeDirectionsReducer(
          mapNode,
          action
        )
...
const MapNodesDirectionsDispatchContext = React.createContext<
