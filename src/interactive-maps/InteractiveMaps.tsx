...
import { mapNodesStateManager, mapNodesDirectionsStateManager } from './map-nodes';
  ...
  if (child.props) {
    ...
    if (typeof children !== 'string' && React.Children.count(children) > 0) {
      React.Children.forEach(children, (child) => {
        return createAreasByID(child as AreaElement, floorID, includedAreas, areaType);
      });
    }
  }
  return includedAreas;
...
const removeLastCharacterString = (str: string) => str.substring(0, str.length - 1);
...
const useDefaultNav = (
  ...
  React.useMemo(() => {
    const startpoint = mapAreas.find((area) => area.value.id === defaultStartingPoint);
    if (startpoint) {
      const endpoint = {
        label: '',
        value: {
...
const MapsDataSource: React.FC<{
  ...
  const { floors: enhancedFloors, areas: mapAreas } = useDataSourceForInteractiveMap(
    dataSource,
  );
