...
const generateDirectionPhrase = (() => {
  ...
  const createPhraseWithDirection = (phrase: string) => (direction: DirectionType) =>
  ...
  const getPhrase = (idx: number, direction: DirectionType) => phrases[idx](direction);
  return (direction: DirectionType): string => {
    ...
    if (!previousRandomNumber) {
      ...
      if (previousRandomNumber === randomNumber || randomNumber === phrases.length) {
...
const useCreateSpeechCollection = ({
  ...
  mapAreasObj: Record<string, types.Area>;
  ...
  return React.useCallback(
    ({
      ...
      if (route.routeInvolvesMultipleFloors) {
        ...
        const portalName = mapAreasObj[portalID].label.split(' ')[0];
      ...
      const startpointAreaType = mapAreasObj[route.startpoint].value.type;
      const endpointAreaName = mapAreasObj[route.endpoint].label;
      ...
      if (wayfinderDirections && wayfinderDirections.length > 0) {
        ...
        const lastEndPhrase = `You can find ${attachedPhraseIntoLastEndPhrase} on the ${wayfinderDirections[lastIndex]} hand side.`;
        let endSpeech = '';
        if (distance) {
...
const VoiceAssistant: React.FC<{
  ...
  ({
    ...
    const [speechCollection, setSpeechCollection] = React.useState<ReadonlyArray<string>>(
      [],
    );
    const [speechAndWayfinderStatus, speechAndWayfinderStatusDispatch] = React.useReducer(
      speechAndWayfinderStatusReducer,
      initStatusState,
    );
    const navigationDispatch = nav.stateManager.useNavigationDispatch();
    const { welcomeSpeech } = appSetters;
    const areasObj = areas.stateManager.useAreasToObj();
    const { endpoint } = nav.stateManager.useNavigation();
    const endpointID = endpoint.value.id;
    const mapAreasObj = areas.stateManager.useAreasToObj();
    ...
    const activeFloor = floorsObj[endpoint.value.floorID || ''];
    ...
    const audioDelay = React.useCallback(() => wayfinderDistance, [wayfinderDistance]);
      ...
      function subscribeToNewNavigation() {
        ...
        const subscription = navigationObservable.subscribe(({ type }) => {
          if (type === 'new') {
            ...
            // ----------- Speech Collection ------------- //
            const endArea = mapAreasObj[endpointID].label;
            const introSpeech = `You can find ${endArea} on the ${activeFloor.label}.`;
    ...
    React.useEffect(
      function subscribeToWayfinder() {
        const endSubscription = wayfinderObservables.end.subscribe(() => {
          ...
          if (endpoint.value.floorID !== route.floorID) {
      ...
      [endpoint.value.floorID, route.floorID, wayfinderObservables.end],
    );
    ...
    React.useEffect(
      function settingNavigationStartingPoint() {
        if (
          ...
          const startpointArea = areasObj[shortestPortal.portal];
          navigationDispatch({
            type: 'START_POINT',
            payload: {
              ...startpointArea,
              value: merge(startpointArea.value, {
                // Assigned the nextFloorID to next starting point.
          ...
          const mapNodesObj = utils.nodes.createMapNodesObj(_floors, activeFloorID);
